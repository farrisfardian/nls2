(function () {
    ...
    angular.module('BlurAdmin')
            .controller('ProvitCtrl', ProvitCtrl)
            .controller('ProvitModalController', ProvitModalController);

    /** @ngInject */
    function ProvitCtrl($scope, $uibModal, $log, $filter, toastr, ProvitService, KotaService, PelayaranService, EnumService, SatuanKirimService, KategoriBarangService) {
        $scope.search = "";
        $scope.oldSearch = "";
        ...
        $scope.reloadData = function () {
            $scope.dataPage = ProvitService.queryComposite(
...
//            $scope.dataPage = ProvitService.query($scope.search, $scope.paging.currentPage - 1, function () {
//                $scope.paging.maxSize = ($scope.dataPage.size);
//                $scope.paging.totalItems = $scope.dataPage.totalElements;
//                $scope.paging.currentPage = parseInt($scope.dataPage.number) + 1;
//                $scope.paging.maxPage = $scope.dataPage.totalPages;
//            });
        ...
        $scope.baru = function () {
            ...
            $scope.ori = {};
            $scope.modalTitle = "Tambah Setting Provit";
            console.log('Baru');
        };
        ...
        $scope.update = function (x) {
            ...
            $scope.ori = {};
            $scope.modalTitle = "Tambah Setting Provit";
            ProvitService.cariSatu("kode", x.id).success(function (data) {
        ...
        $scope.edit = function (x) {
            ...
            $scope.modalTitle = "Edit Setting Provit";
            console.log('edit', x);
            ProvitService.cariSatu("kode", x.id).success(function (data) {
        ...
        $scope.hapus = function (x) {
            ProvitService.hapus(x.id).success(function () {
                toastr.success('Hapus data sukses!');
                $scope.reloadData();
            }).error(function () {
                toastr.error('Maaf Anda tidak dapat menghapus Setting Provit ini!');
            });
        };
        $scope.opened = false;
        ...
        $scope.simpan = function () {
            ProvitService.simpan($scope.vm, $scope.ori).success(function (d) {
                toastr.success('Simpan data sukses!');
                $scope.clear();
            }).error(function (e) {
                toastr.error('Gagal simpan Setting Provit! Pastikan tanggal & asal tidak sama persis dengan setting yang lalu.');
            });
        };
...
//                    window.open(link, '_blank', 'width=screen.width, height=screen.height');
    ...
    function ProvitModalController($uibModalInstance, toastr, $scope, ProvitService, KotaService, PelayaranService, KapalService, EnumService, SatuanKirimService, JenisItemService, data) {
