(function () {
    ...
    angular.module('BlurAdmin.pages.transaksi')
            .controller('RekapNotaCtrl', RekapNotaCtrl)
            ;

    /** @ngInject */
    function RekapNotaCtrl($scope, $uibModal, $log, $filter, toastr, NotaService, EnumService, TokoService, KotaService, KapalService) {
        $scope.search = "";
        $scope.oldSearch = "";
        ...
        $scope.cetak = function (tipe) {
            var link = 'api/report/get-rekap-nota-tagihan.' + tipe
                    + '?idToko=' + ($scope.param.tokoTujuan === null ? 'null' : $scope.param.tokoTujuan.id)
                    + "&toko=" + ($scope.param.tokoTujuan === null ? 'null' : $scope.param.tokoTujuan.nama)
                    + "&idMerk=" + ($scope.param.merkTujuan === null ? 'null' : $scope.param.merkTujuan.id)
                    + "&merk=" + ($scope.param.merkTujuan === null ? 'null' : $scope.param.merkTujuan.nama)
                    + "&idKapal=" + ($scope.param.kapal === null ? 'null' : $scope.param.kapal.id)
                    + "&kapal=" + ($scope.param.kapal === null ? 'null' : $scope.param.kapal.nama)
                    + "&idKotaTujuan=" + ($scope.param.kota === null ? 'null' : $scope.param.kota.id)
                    + "&kotaTujuan=" + ($scope.param.kota === null ? 'null' : $scope.param.kota.nama)
