(function () {
    ...
    angular.module('BlurAdmin.pages.transaksi')
            .controller('RekapNotaBelumLunasCtrl', RekapNotaBelumLunasCtrl)
            ;

    /** @ngInject */
    function RekapNotaBelumLunasCtrl($scope, $uibModal, $log, $filter, toastr, NotaService, EnumService, TokoService) {
        $scope.search = "";
        $scope.oldSearch = "";
        ...
        $scope.cetak = function (tipe) {
            var link = 'api/report/rekap-nota-belum-lunas.' + tipe + '?idToko=null&idMerk=' + ($scope.param.merkTujuan === null ? 'null' : $scope.param.merkTujuan.id) + "&tglAwal=" + $filter('date')(new Date($scope.param.tglAwal), 'yyyy-MM-dd') + "&tglAkhir=" + $filter('date')(new Date($scope.param.tglAkhir), 'yyyy-MM-dd');
            if (tipe == 'pdf') {
