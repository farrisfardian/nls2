(function () {
    ...
    function NotaCtrl($scope, $uibModal, $log, $filter, $stateParams, $window, toastr, NotaService, KapalBerangkatService, EnumService, TokoService, TambahanBiayaService, KotaService) {
        ...
        $scope.reloadData = function () {
            ...
            $scope.dataPage = NotaService.queryGetTagihanTerbayar($filter('date')(new Date($scope.param.tglAwal), 'yyyy-MM-dd'), $filter('date')(new Date($scope.param.tglAkhir), 'yyyy-MM-dd'), ($scope.param.cari === '' ? 'null' : $scope.param.cari.replace(/\//g, "~")), ($scope.param.toko === null ? 'null' : $scope.param.toko.id), ($scope.param.merk === null ? 'null' : $scope.param.merk.id), ($scope.param.status === 'Semua' ? "null" : $scope.param.status), $scope.paging.currentPage - 1, function () {
