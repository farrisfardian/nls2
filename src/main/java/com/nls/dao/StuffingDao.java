...
public interface StuffingDao extends PagingAndSortingRepository<Stuffing, Integer> {
    ...
    @Query("from Stuffing a where upper(coalesce(a.noKontainer,'')) like :search or upper(coalesce(a.emkl.nama,'')) like upper(:search) or upper(coalesce(a.kapalBerangkat.kapal.nama,'')) like upper(:search) or upper(coalesce(a.satuanKirim.nama,'')) like upper(:search)")
