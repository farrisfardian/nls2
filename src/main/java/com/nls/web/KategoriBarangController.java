...
import com.nls.dao.KategoriBarangDao;
...
@CrossOrigin
@RestController
@RequestMapping("/api/master/kategori-barang")
public class KategoriBarangController {

    @Autowired
    KategoriBarangDao dao;

//    @Autowired
//    AppService appService;
    private final Logger logger = LoggerFactory.getLogger(KategoriBarangController.class);

    @RequestMapping(value = "/all", method = RequestMethod.GET)
    public Iterable<KategoriBarang> cariSemua() {
        return dao.findAll();
    }

    @RequestMapping(method = RequestMethod.GET)
    public Page<KategoriBarang> saringSemua(
        ...
        Page<KategoriBarang> result = dao.findAll(pr);
        return result;
    }

    @RequestMapping(value = "{column}/{value}", method = RequestMethod.GET)
    public KategoriBarang cariSatu(@PathVariable String column, @PathVariable String value) {
    ...
    @ResponseBody
    public Page<KategoriBarang> cariBerdasarkanNama(@PathVariable("nama") String nama,
            Pageable pageable,
    ...
    public void hapus(@PathVariable String id) {
        KategoriBarang x = dao.findOne(Integer.valueOf(id));
        if (x == null) {
    ...
    public void simpan(@RequestBody KategoriBarang x) {
//        User u = appService.getCurrentUser();
        dao.save(x);
    }

    @RequestMapping(value = "{id}", method = RequestMethod.PUT)
    public void perbarui(@PathVariable String id, @RequestBody KategoriBarang x) {
        KategoriBarang r = dao.findOne(Integer.valueOf(id));
        if (r == null) {
            throw new InvalidParameterException("KategoriBarang dengan nama '" + x.getNama() + "' tidak ditemukan!");
        }
        x.setId(r.getId());
        dao.save(x);
    }
