...
import com.nls.dao.PelayaranDao;
...
@CrossOrigin
@RestController
@RequestMapping("/api/master/pelayaran")
public class PelayaranController {
...
//    AppService appService;
    private final Logger logger = LoggerFactory.getLogger(PelayaranController.class);

    @RequestMapping(value = "/all", method = RequestMethod.GET)
    public Iterable<Pelayaran> cariSemua() {
        return dao.findAll();
    }

    @RequestMapping(method = RequestMethod.GET)
    public Page<Pelayaran> saringSemua(
        ...
        Page<Pelayaran> result = dao.findAll(pr);
        return result;
    }

    @RequestMapping(value = "{column}/{value}", method = RequestMethod.GET)
    public Pelayaran cariSatu(@PathVariable String column, @PathVariable String value) {
    ...
    @ResponseBody
    public Page<Pelayaran> cariBerdasarkanNama(@PathVariable("nama") String nama,
            Pageable pageable,
    ...
    public void hapus(@PathVariable String id) {
        Pelayaran x = dao.findOne(Integer.valueOf(id));
        if (x == null) {
    ...
    public void simpan(@RequestBody Pelayaran x) {
//        User u = appService.getCurrentUser();
        dao.save(x);
    }

    @RequestMapping(value = "{id}", method = RequestMethod.PUT)
    public void perbarui(@PathVariable String id, @RequestBody Pelayaran x) {
        Pelayaran r = dao.findOne(Integer.valueOf(id));
        if (r == null) {
            throw new InvalidParameterException("Pelayaran dengan nama '" + x.getNama() + "' tidak ditemukan!");
        }
        x.setId(r.getId());
        dao.save(x);
    }
