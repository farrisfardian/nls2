...
import com.nls.dao.RoleDao;
import com.nls.domain.Role;
import com.nls.domain.RoleMenu;
...
@CrossOrigin
@RestController
@RequestMapping("/api/setting/role")
...
//    AppService appService;
    ...
    public Iterable<Role> cariSemua() {
        return dao.findAll();
    }
    
    @RequestMapping(method = RequestMethod.GET)
    public Page<Role> saringSemua(
        ...
        Page<Role> result = dao.findAll(pr);
        return result;
    }
    
    @RequestMapping(value = "{column}/{value}", method = RequestMethod.GET)
    public Role cariSatu(@PathVariable String column, @PathVariable String value) {
    ...
    @ResponseBody
    public Page<Role> cariBerdasarkanNama(@PathVariable("nama") String nama,
            Pageable pageable,
    ...
    public void simpan(@RequestBody Role x) {
//        User u = appService.getCurrentUser();
        if (x.getMenuSet() != null) {
            for (RoleMenu y : x.getMenuSet()) {
                y.setRole(x);
            }
        }
        dao.save(x);
    ...
    public void perbarui(@PathVariable String id, @RequestBody Role x) {
        Role r = dao.findOne(id);
        if (r == null) {
            throw new InvalidParameterException("Role dengan nama '" + x.getName() + "' tidak ditemukan!");
        }
        if (x.getMenuSet() != null) {
            for (RoleMenu y : x.getMenuSet()) {
                y.setRole(x);
